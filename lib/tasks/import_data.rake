namespace :nba do
  desc "Import 2025-26 NBA season data for players averaging 10+ PPG"
  task import_2025_26: :environment do
    season = "2025-26"
    season_start = Date.parse("2025-10-21")
    min_ppg = 10
    ...
    players_data.each_with_index do |player_data, index|
      ...
      player.update!(team: player_data[:team]) if player.team != player_data[:team]
      ...
      # Create game logs (skip duplicates)
      game_logs_created = 0
      game_logs_data.each do |log_data|
        week_number = calculate_week_number(log_data[:game_date], season_start)

        # Skip if already exists
        next if GameLog.exists?(player: player, game_date: log_data[:game_date], season: season)
    ...
    Player.find_each do |player|
      ...
      weeks.each do |week|
        # Skip if already exists
        next if WeeklyHigh.exists?(player: player, week_number: week, season: season)
    ...
    Player.find_each do |player|
      ...
      existing_summary = PlayerSummary.find_by(player: player, season: season)
      if existing_summary
        puts "  ⚠️  Summary already exists for #{player.name}, skipping"
        next
      end
      ...
      # Calculate avg_score and games_played from all game logs
      all_games = player.game_logs.where(season: season)
      avg_score = all_games.any? ? all_games.average(:fantasy_score).to_f : 0
    ...
    puts "  Game Logs (#{season}): #{GameLog.where(season: season).count}"
    puts "  Weekly Highs (#{season}): #{WeeklyHigh.where(season: season).count}"
    puts "  Player Summaries (#{season}): #{PlayerSummary.where(season: season).count}"
    puts "=" * 60
  end

  desc "Import 2024-25 NBA season data for players averaging 10+ PPG"
  task import_2024_25: :environment do
    season = "2024-25"

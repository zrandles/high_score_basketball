...
# config/deploy.rb
set :application, 'high_score_basketball'
set :repo_url, 'git@github.com:zrandles/high_score_basketball.git'
set :branch, 'main'
...
namespace :deploy do
  ...
  task :restart_puma_sudo do
    on roles(:app) do
      execute :sudo, :systemctl, :restart, 'high_score_basketball.service'
    end
  end
